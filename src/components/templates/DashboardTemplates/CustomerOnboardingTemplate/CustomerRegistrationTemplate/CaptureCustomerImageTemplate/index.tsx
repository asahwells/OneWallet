...
interface TakePhotoStepProps {
  onContinue?: (photo: string) => void; // optional callback with the captured photo
  onBack: () => void;                  // callback to go back
}

const videoConstraints = {
  facingMode: 'user', // "user" for front camera on mobile
};
...
const CaptureCustomerImageTemplate = ({ onContinue, onBack }: TakePhotoStepProps) => {
  ...
  // Call the Next.js API route which calls Dojah
  ...
  return (
    <>
      ...
      >
        ...
        <Text variant="sm" textAlign={isMobile ? 'left' : 'center'} mb={6}>
        ...
        <Button
          ...
          isDisabled={!hasTakenPhoto || loading}
          isLoading={loading}
